...
HealthKit과 EventKit 네이티브 API를 감싸는 최하위 계층 래퍼다. 이 계층이 없어지면 앱이 Apple 시스템 프레임워크와 직접 결합되어 테스트와 교체가 불가능해진다. `Data/` 계층만 이 계층을 사용한다.
...
- `Helper/` — Core 계층 공통 유틸리티

기술 스택: Swift, HealthKit, EventKit
...
- **피의존**: `Data/`(HealthKitRepositoryImpl, EventKitRepositoryImpl)
