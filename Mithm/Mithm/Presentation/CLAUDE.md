...
- **새 화면 추가**: View + ViewModel 파일 생성 → `AppDependencyGraph`에 프로퍼티 추가 → `AppDIContainer.makeAppDependencyGraph()`에서 조립 → `MithmApp`에서 `environmentObject`로 주입
...
- **ViewModel별 조립 팩토리를 따로 만들지 말 것** — 과거 `makeHomeViewModel(appState:)` 같은 팩토리가 `makeAppDependencyGraph()`와 그래프를 이중으로 조립해, ViewModel에 의존성을 추가하면 두 곳을 고쳐야 하고 한 곳만 고치면 Preview만 조용히 깨졌다. Demo 화면처럼 본 앱 흐름 밖의 화면만 전용 팩토리를 갖는다
- **Demo 화면도 `AppDIContainer`를 거칠 것** — `Demo/` ViewModel이 `RepositoryImpl`/`DataStoreImpl`을 직접 `init`하면 "Data 구현체 생성은 composition root 한 곳" 규칙이 깨진다. Demo ViewModel은 의존성을 생성자로만 받고 `AppDIContainer.makeMenstrualRecordUseCaseDemoViewModel()`이 조립한다
...
- **`MithmApp` 진입점에서의 흐름**: `MithmApp.init()` → `AppDIContainer.makeAppDependencyGraph()`가 `AppState`와 모든 ViewModel을 한 번에 조립 → `@StateObject`로 보관 후 `environmentObject`로 주입. 앱이 쓰는 그래프 조립 경로는 이 하나뿐이며 `#Preview`도 같은 팩토리를 쓴다
