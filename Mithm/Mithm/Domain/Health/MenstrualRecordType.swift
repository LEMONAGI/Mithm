...
enum MenstrualRecordType {
    case menstrualRecord             // 과거 월경 실제 기록
    
    case ovulationEstimated          // 과거 배란일 추정
    case ovulationFertileWindowEstimated   // 과거 기록 기반 배란기 추정
    
    case menstrualPrediction         // 미래 월경 예측
    case ovulationPrediction         // 미래 배란일 예측
    case ovulationFertileWindowPrediction  // 미래 배란기 예측
}
...
extension MenstrualRecordType {
    ...
    var title: String {
        ...
        case .menstrualRecord:
            return String(localized: "월경 기록")
        case .ovulationEstimated:
            return String(localized: "배란일(추정)")
        case .ovulationFertileWindowEstimated:
            return String(localized: "배란기(추정)")
        case .menstrualPrediction:
            return String(localized: "월경 예정 기간")
        case .ovulationPrediction:
            return String(localized: "배란일(예상)")
        case .ovulationFertileWindowPrediction:
            return String(localized: "배란기(예상)")
        }
    }
    ...
    var notes: String? {
        switch self {
        case .menstrualRecord:
            return String(localized: "사용자 기록에 기반한 실제 월경 기간입니다.")
        case .ovulationEstimated:
            return String(localized: "앱이 건강 기록을 기반으로 사후적으로 추정한 배란일입니다.")
        case .ovulationFertileWindowEstimated:
            return String(localized: "월경 기록을 기반으로 추정한 배란기(가임기)입니다.")
        case .menstrualPrediction:
            return String(localized: "앱에서 예측한 월경 예정 기간입니다.")
        case .ovulationPrediction:
            return String(localized: "앱에서 예측한 배란일입니다.")
        case .ovulationFertileWindowPrediction:
            return String(localized: "예측된 월경 예정일을 바탕으로 계산한 배란기(가임기)입니다.")
        }
    }
