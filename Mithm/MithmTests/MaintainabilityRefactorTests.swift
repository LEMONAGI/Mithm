...
struct PhaseDetailLocalizationTests {
    ...
    @Test("변경된 PhasePresentation 문구는 영어 로컬라이제이션을 가진다")
    func updatedPhasePresentationCopyHasEnglishLocalization() throws {
        let expectedLocalizations = [
            "모든 호르몬이 낮아지며 몸이 휴식 모드가 돼요\n한 주기를 마무리하는 시간이에요",
            "에스트로겐이 증가하며 에너지와 집중이 올라요\n리듬이 서서히 깨어나기 시작해요",
            "에스트로겐이 최고조에 달해 에너지가 높아져요\n몸의 리듬이 안정적으로 맞물려요",
            "프로게스테론이 높아지며 휴식의 신호를 보내요\n리듬이 느려지면서 에너지가 차츰 낮아져요",
            "에스트로겐과 프로게스테론이 동시에 낮아지면서 자궁 내막이 떨어져 나가고 월경이 시작됩니다. 이때 호르몬 수치가 최저점으로 급격히 떨어지면서 신체적인 피로감이 커지고, 자궁을 수축시키는 물질(프로스타글란딘) 때문에 아랫배나 허리에 통증이 찾아오게 돼요. 기분이 가라앉고 무기력한 건 마음의 문제가 아니라, 몸이 새로운 주기를 준비하느라 꽤 많은 에너지를 쓰고 있기 때문이랍니다.",
            "지금은 몸이 스스로 리셋하는 시기예요. 억지로 평소 페이스를 유지하려 하기보단, 조금 느슨하게 가는 것이 오히려 다음 주기를 잘 시작하는 방법이에요.",
            ...
            "새로운 프로젝트나 공부를 시작하기에 좋은 시기예요. 이 에너지가 있을 때 첫 발을 떼보세요.",
            ...
            "몸이 가벼워진 만큼 마음에도 긍정적인 활력과 생기가 도는 시기예요. 감정의 가라앉음이 덜하고 컨디션이 받쳐주다 보니, 평소보다 대화나 대외 활동을 할 때 조금 더 편안하고 자연스럽게 나를 표현할 수 있어요.",
            ...
            "에너지가 넘친다고 해서 몸을 과도하게 무리해서 쓰기보다는, 곧 찾아올 황체기(생리 전 시기)를 고려해 기분 좋은 활력을 즐기며 적절한 휴식도 함께 챙겨두세요.",
            "배란 후 황체에서 프로게스테론이 분비되기 시작해요. 프로게스테론은 자궁 내막을 안정시키는 역할을 하지만, 동시에 프로게스테론이 대사되는 과정에서 생성된 알로프레그나놀론이 GABA-A 수용체에 영향을 미치는 것으로 알려져 있어요. 이로 인해 기분 변화, 불안, 피로감을 유발할 수 있어요. 또한 황체기 후반 에스트로겐이 낮아지면서 세로토닌 합성도 줄어들 수 있어요. 월경이 가까워질수록 에스트로겐과 프로게스테론이 모두 떨어지면서 PMS 증상이 나타나기 쉬운 구간이 돼요.",
            "단것이 당기는 증상이 나타날 수 있어요. 초콜릿 한 조각 정도는 괜찮지만, 폭식보다는 마그네슘·철분이 든 음식이 도움이 돼요."
        ]
        let localizations = try localizableStrings()
